...
Function AddRandomPrivilege($controllee, $controller, $controlleeType, $controllerType)
{
    ...
    $controllerSID = New-Object System.Security.Principal.SecurityIdentifier $controller.objectsid.value
    $controlleeDN = (iex "Get-AD$controlleeType `"$($controllee.Name)`"").DistinguishedName
    $controlleeAcl = get-acl "AD:$($controlleeDN)"

    # Adding additional perms
    if ($controlleeType -eq "User" -or "ServiceAccount")
    {
        ...
        if ($controlleeType -eq "User")
        {
            $privileges += @( (New-Object PSOBject -Property @{Type="Extended";ACL="Validated-SPN";GUID="f3a64788-5306-11d1-a9c5-0000f80367c1"}) )
        }
        if ($controllee.Name.StartsWith("svc_"))
        {
            $privileges += @(
    ...
    if ($controlleeType -eq "Domain")
    {
        $privileges += @(
            (New-Object PSOBject -Property @{Type="DCSYNC";ACL="DCSync";EXTENDEDRIGHTS=@(
                (New-Object PSOBject -Property @{Type="Extended";ACL="DS-Replication-Get-Changes";GUID="1131f6aa-9c07-11d1-f79f-00c04fc2dcd2"}),
                (New-Object PSOBject -Property @{Type="Extended";ACL="DS-Replication-Get-Changes-All";GUID="1131f6ad-9c07-11d1-f79f-00c04fc2dcd2"}),
                (New-Object PSOBject -Property @{Type="Extended";ACL="DS-Replication-Get-Changes-In-Filtered-Set";GUID="89e95b76-444d-4c62-991a-0facbeda640c"}))}
            )
        )                    
    }
    ...
    {
        $controlleeAcl.AddAccessRule( (New-Object System.DirectoryServices.ActiveDirectoryAccessRule $controllerSID,([System.DirectoryServices.ActiveDirectoryRights]$privilege.ACL),"Allow",([System.DirectoryServices.ActiveDirectorySecurityInheritance] "None")) )
        set-acl -aclobject $controlleeAcl -path "AD:$($controlleeDN)"
    }
    elseif ($privilege.Type -eq "Extended")
    {
        $controlleeAcl.AddAccessRule( (New-Object System.DirectoryServices.ExtendedRightAccessRule $controllerSID, "Allow", $privilege.GUID, ( [System.DirectoryServices.ActiveDirectorySecurityInheritance] "None") ) )
        set-acl -aclobject $controlleeAcl -path "AD:$($controlleeDN)"
    }
    elseif ($privilege.Type -eq "DCSYNC")
    {
        foreach ($dcsyncPriv in $privilege.EXTENDEDRIGHTS)
        {
        $controlleeAcl.AddAccessRule( (New-Object System.DirectoryServices.ExtendedRightAccessRule $controllerSID, "Allow", $dcsyncPriv.GUID, ( [System.DirectoryServices.ActiveDirectorySecurityInheritance] "None") ) )
        }
        set-acl -aclobject $controlleeAcl -path "AD:$($controlleeDN)"
    }
        ...
        {$_ -eq "Constrained"}
            {
                ...
                iex "Set-AD$controllerType -Identity `"$($controller.Name)`" -Add @{ 'msDS-AllowedToDelegateTo'=@(`"$($controlleeSPN)`") }"
            }
